...
**This changes the default for every existing SAML application.** A deployment that relied on Logto re-authenticating at each SAML application now needs the service provider to send `ForceAuthn="true"`, which is the standard way to ask an identity provider for re-authentication. There is no Logto-side setting for this yet.
